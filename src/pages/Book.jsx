...
const FIELD =
  'w-full border-b border-bone/20 bg-transparent px-0 py-3 text-body text-bone ' +
      ...
      {labels.map((label, i) => {
        ...
        return (
          <li key={label} className="flex items-center gap-3">
            ...
            {i < labels.length - 1 && <span aria-hidden="true" className="h-px w-6 bg-bone/20" />}
          </li>
        );
      })}
    </ol>
  ...
  return (
    <p
      ...
      className={`border-l-2 py-2 pl-4 text-sm ${
        tone === 'error' ? 'border-[#C4644A] text-[#E4917A]' : 'border-bone/25 text-linen'
      }`}
    >
      {children}
    </p>
      ...
      <section className="shell pb-section">
        <div className="grid gap-14 lg:grid-cols-12">
          <div className="lg:col-span-7">
            {/* ---------- 1. dates ---------- */}
            {step === 1 && (
              <form onSubmit={goToRooms} className="border border-bone/10 bg-espresso p-8 lg:p-10">
            ...
            {/* ---------- 2. room ---------- */}
            {step === 2 && (
              <div className="border border-bone/10 bg-espresso p-8 lg:p-10">
                <h2 className="font-display text-d3">Choose a room</h2>
                <p className="prose-body mt-2 text-sm">
                  {stayNights} {stayNights === 1 ? 'night' : 'nights'} at {property?.name}.
                </p>

                {busy && <p className="mt-6 text-sm text-linen">Checking availability…</p>}
                  ...
                  {rows.map((row) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex flex-wrap items-baseline justify-between gap-x-6 gap-y-2 border-t border-bone/10 py-5 text-left transition-colors duration-400 ${
                          soldOut ? 'cursor-not-allowed opacity-40' : 'hover:bg-bone/[0.05]'
                        }`}
            ...
            {/* ---------- 3. details ---------- */}
            {step === 3 && (
              <form onSubmit={submit} className="border border-bone/10 bg-espresso p-8 lg:p-10">
                ...
                <div className="border border-bone/10 bg-espresso p-8 lg:p-10">
                  ...
                  <dl className="mt-8 flex flex-col">
                    {[
                      ...
                      <div key={k} className="flex items-baseline justify-between gap-6 border-t border-bone/10 py-3">
          ...
          <aside className="lg:col-span-4 lg:col-start-9">
            <div className="sticky top-28 border-t border-bone/15 pt-6">
              ...
              {chosenRow && stayNights > 0 && (
                <div className="mt-5 flex items-baseline justify-between gap-4 border-t border-bone/15 pt-5">
